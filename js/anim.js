...
// Получаем все элементы path с классом wave
const waves2 = document.querySelectorAll('.wave2');

// Для каждой волны создаем анимацию
waves2.forEach((wave2, index) => {
  // Устанавливаем начальное значение stroke-dasharray для текущей волны
  const length = wave2.getTotalLength();
  wave2.style.strokeDasharray = `${length} ${length}`;
  wave2.style.strokeDashoffset = length;
...
waves4.forEach((wave4, index) => {
  ...
  wave4.style.strokeDashoffset = length;
